...
const int
   num_productor          = 4,
   id_buffer             = num_productor,
   num_consumidor         = 5,
   num_procesos_esperado = num_productor + num_consumidor + 1,
   num_items             = 20,
   tam_vector            = 10,
   id_productor           = 0,
   id_consumidor          = 1;
   ...
   //****************************************************************************//
   //***                           COMPILAR                                   ***//
   //***      mpicxx -std=c++11 -o prodcons2-mu_exe prodcons2-mu.cpp        ***//
   //***                                                                      ***//
   //***              mpirun -np 10 ./prodcons2-mu_exe                       ***//
   //****************************************************************************//
...
int producir(int num_p)
{
   ...
   int producido = (num_p*(num_items/num_productor)+contador);
   cout << "Productor " << num_p << " ha producido valor " << producido << endl << flush;
   contador++ ;
   return producido ;
}
// ---------------------------------------------------------------------
...
{
   for (int i= 0 ; i < num_items / num_productor ; i++ )
   {
      // producir valor
...
void funcion_consumidor(int num_c)
{
   ...
   for( int i=0 ; i < num_items / num_consumidor; i++ )
   {
...
void funcion_buffer()
{
   int        buffer[tam_vector],      // buffer con celdas ocupadas y vacías
              ...
              tag ;    // identificador de emisor aceptable
   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      if ( num_celdas_ocupadas == 0 )               // si buffer vacío
         tag = id_productor ;       // $~~~$ solo prod.
      else if ( num_celdas_ocupadas == tam_vector ) // si buffer lleno
         tag = id_consumidor ;      // $~~~$ solo cons.
      ...
      switch( estado.MPI_TAG ) // leer emisor del mensaje en metadatos
      {
         ...
         case id_consumidor: // si ha sido el consumidor: extraer y enviarle
            ...
            MPI_Ssend( &valor, 1, MPI_INT, estado.MPI_SOURCE, id_consumidor, MPI_COMM_WORLD);
            break;
      }
   }
}
...
int main( int argc, char *argv[] )
{
   ...
   if (id_propio < num_productor)
      n_orden = id_propio;
   else if (id_propio > num_productor)
      n_orden = id_propio % (num_productor + 1);
