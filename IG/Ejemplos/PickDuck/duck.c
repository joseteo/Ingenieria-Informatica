...
int pick(int x, int y,int * i, int * parte)
{
   ...
   glRenderMode (GL_SELECT);     // Paso a modo seleccion
   ...
   if(hits>0)
	...
	b=0;
 	Zmin = -1;
for(k=0;k<hits;++k){    // Determinar el más próximo
	if(buff[b]!=0){
		printf("\n pato: %u con %u nombres  Z: (%u %u)  Zmin: %i", buff[b+3], buff[b],buff[b+1],buff[b+2],Zmin);
		z = buff[b+1];
		if(Zmin==-1 || z<Zmin) {
			*i=buff[b+3];
			*parte=buff[b+4];
   ...
   printf("\n pato: %i en %i Zmin %i",*i,*parte,Zmin);
   }
setviewtrans();
return *i;
}
