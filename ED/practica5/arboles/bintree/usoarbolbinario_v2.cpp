...
int ContarHojas(bintree<T>& a){
	...
	int  cont=0;
	for (typename bintree<T>::preorder_iterator i = a.begin_preorder(); i!=a.end_preorder(); ++i){
...
int caminos (bintree<int> &a, int k, bintree<int>::node sa){

    if (sa.null())
      return 0;
    else if(!a.esHoja(sa))
      return caminos(a, k - *sa, sa.left()) + caminos(a, k - *sa, sa.right());
    else if( *sa ==k)
      return 1;
    else
      return 0;
...
int longitud=20;
cout << "Caminos:  " << caminos( Arb, longitud, Arb.root()) << endl;
...
//n caminos con un n etiquetas cuyos valores sumen k
